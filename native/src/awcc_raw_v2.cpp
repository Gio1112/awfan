...
public:
    ComRuntime() {
        ...
        if (FAILED(result_)) {
            throw std::runtime_error("CoInitializeEx failed (" + hresult_text(result_) + ")");
        }
...
void print_response(
    ...
    std::cout
        ...
        << std::dec << std::setfill(' ')
        << " signed=" << static_cast<std::int32_t>(response)
        << " bytes=["
...
public:
    AwccClient() {
        ...
        if (FAILED(result)) {
            throw std::runtime_error("CoCreateInstance failed (" + hresult_text(result) + ")");
        }
        ...
        if (FAILED(result)) {
            throw std::runtime_error("ConnectServer failed (" + hresult_text(result) + ")");
        }
        ...
        if (FAILED(result)) {
            throw std::runtime_error("GetObject failed (" + hresult_text(result) + ")");
        }
    ...
    std::uint32_t call(
        ...
        if (FAILED(result) || input_parameters == nullptr) {
            throw std::runtime_error("GetMethod failed (" + hresult_text(result) + ")");
        }

        VARIANT parameters{VT_I4};
        parameters.uintVal = pack_argument(subcommand, argument1, argument2);
        result = input_parameters->Put(L"arg2", 0, &parameters, 0);
        if (FAILED(result)) {
            throw std::runtime_error("Put(arg2) failed (" + hresult_text(result) + ")");
        }
        ...
        if (FAILED(result)) {
            throw std::runtime_error("ExecMethod failed (" + hresult_text(result) + ")");
        }

        VARIANT output{VT_I4};
        output.intVal = -1;
        result = output_parameters->Get(L"argr", 0, &output, nullptr, nullptr);
        if (FAILED(result)) {
            VariantClear(&output);
            throw std::runtime_error("Get(argr) failed (" + hresult_text(result) + ")");
        }
...
private:
    std::wstring find_instance_path() const {
        ...
        if (FAILED(result)) {
            throw std::runtime_error("CreateInstanceEnum failed (" + hresult_text(result) + ")");
        }

        ComPtr<IWbemClassObject> instance;
        ULONG returned = 0;
        result = enumerator->Next(10000, 1, instance.GetAddressOf(), &returned);
        if (FAILED(result) || returned == 0 || instance == nullptr) {
            throw std::runtime_error("No AWCC instance returned (" + hresult_text(result) + ")");
        }

        VARIANT path;
        VariantInit(&path);
        result = instance->Get(L"__Path", 0, &path, nullptr, nullptr);
        if (FAILED(result) || path.vt != VT_BSTR || path.bstrVal == nullptr) {
            VariantClear(&path);
            throw std::runtime_error("Could not read AWCC __Path (" + hresult_text(result) + ")");
        }
...
void print_read(
    ...
    const std::uint32_t request = pack_argument(subcommand, argument1, argument2);
    ...
    print_response(label, request, response);
}
...
int run_awcc_raw_probe() {
    ...
    std::cout << "All operations below are read-only.\n\n";

    print_read(client, "system-id", L"Thermal_Information", 0x02, 0x02);
    ...
    for (std::uint8_t index = 0; index < 64; ++index) {
        ...
        print_response("  index " + std::to_string(index), request, response);

        const std::int32_t signed_response = static_cast<std::int32_t>(response);
        if (signed_response <= 0) break;

        const std::uint8_t id = static_cast<std::uint8_t>(response & 0xffU);
        if (response > 0x100U && response < 0x110U) {
            sensor_ids.push_back(id);
        } else if (response > 0x8fU) {
    ...
    print_read(client, "  power", L"Thermal_Information", 0x0b);

    std::cout << "\nfan reads:\n";
    for (const std::uint8_t fan_id : fan_ids) {
        const std::string prefix = "  fan 0x" + [&] {
            std::ostringstream stream;
            stream << std::hex << std::uppercase << static_cast<unsigned int>(fan_id);
            return stream.str();
        }();

        print_read(client, prefix + " sensor", L"GetFanSensors", 0x02, fan_id);
        print_read(client, prefix + " rpm", L"Thermal_Information", 0x05, fan_id);
        print_read(client, prefix + " max-rpm", L"Thermal_Information", 0x09, fan_id);
        print_read(client, prefix + " percent", L"Thermal_Information", 0x06, fan_id);
        print_read(client, prefix + " boost", L"Thermal_Information", 0x0c, fan_id);
    }

    std::cout << "\ntemperature reads:\n";
    for (const std::uint8_t sensor_id : sensor_ids) {
        std::ostringstream label;
        label << "  sensor 0x" << std::hex << std::uppercase
